...
class MagTouchSerialReader:
        ...
        for sensor_idx in range(self.config.NUM_SENSORS):
            if self.config.MODEL_NAMES[sensor_idx]:
                self.taxel_models[sensor_idx] = (dill.load(
                    open(os.path.join(python_src_root, f'calibration/magtouch/models/{self.config.MODEL_NAMES[sensor_idx]}'),
                         'rb')))
    ...
    def calibrate(self):
        ...
        self.init_prev_vals()
        means_tmp = np.zeros((self.config.WINDOW_SIZE, self.config.NUM_SENSORS, self.config.NUM_TAXELS, 3))
    ...
    def init_prev_vals(self):
        ...
        for i in range(self.config.NUM_TAXELS):
            ...
            self.prev_y[i] = ~ ((data_bytes[i * 3 * 2 + 2] << 8) + data_bytes[i * 3 * 2 + 3])
            self.prev_z[i] = ~ ((data_bytes[i * 3 * 2 + 4] << 8) + data_bytes[i * 3 * 2 + 5])
    ...
    def read_sample(self):
        ...
        while True:
            ...
            if data == b'\xAA':
                data_bytes = self.ser.read(self.config.NUM_SENSORS * 6 * self.config.NUM_TAXELS)
                for sensor_idx in range(self.config.NUM_SENSORS):
                    for taxel_idx in range(self.config.NUM_TAXELS):
                        ...
                        if abs(x - self.prev_x[taxel_idx]) >= 2 ** 11:
                            ...
                            logger.warning(f'sensor {sensor_idx} taxel {taxel_idx} axis x overflow diff: {x - self.prev_x[taxel_idx]} ; x: {x} ; prev_x: {self.prev_x[taxel_idx]}')
                        if abs(y - self.prev_y[taxel_idx]) >= 2 ** 11:
                            ...
                            logger.warning(f'sensor {sensor_idx} taxel {taxel_idx} axis y overflow diff: {y - self.prev_y[taxel_idx]} ; y: {y} ; prev_y: {self.prev_y[taxel_idx]}')
                        if abs(z - self.prev_z[
                            taxel_idx]) >= 2 ** 11:  # with temp compensation on, the z-overflow is around 2**12 for some reason
                            if not self.cycle_z[taxel_idx]:
                                self.cycle_z[taxel_idx] = z - self.prev_z[taxel_idx]
                            else:
                                self.cycle_z[taxel_idx] = 0
                            logger.warning(f'sensor {sensor_idx} taxel {taxel_idx} axis z overflow diff: {z - self.prev_z[taxel_idx]} ; z: {z} ; prev_z: {self.prev_z[taxel_idx]}')
    ...
    def run(self):
        taxels = np.array([[MagTouchTaxel(x=0, y=0, z=0) for _ in range(self.config.NUM_TAXELS)] for __ in
                           range(self.config.NUM_SENSORS)])  # preallocate
        taxels_raw = np.array([[MagTouchTaxel(x=0, y=0, z=0) for _ in range(self.config.NUM_TAXELS)] for __ in
                           range(self.config.NUM_SENSORS)])  # preallocate
        magtouch_data = [MagTouch4(taxels[i]) for i in range(self.config.NUM_SENSORS)]
        magtouch_raw_data = [MagTouch4(taxels_raw[i]) for i in range(self.config.NUM_SENSORS)]
        self.init_prev_vals()
        while True:
            ...
            for sensor_idx in range(self.config.NUM_SENSORS):
                ...
                if self.taxel_models[sensor_idx]:
                    ...
                    for taxel_idx in range(self.config.NUM_TAXELS):
                        ...
                        taxels[sensor_idx, taxel_idx] = MagTouchTaxel(x=predicted_x, y=predicted_y, z=predicted_z)
                        taxels_raw[sensor_idx, taxel_idx] = MagTouchTaxel(x=raw_x, y=raw_y, z=raw_z)
                    ...
                    self.raw_data_publishers[sensor_idx].publish_sensor_data(magtouch_raw_data[sensor_idx])
                else:
                    for taxel_idx in range(self.config.NUM_TAXELS):
                        raw_x = sample_raw[sensor_idx, taxel_idx, 0]
                        raw_y = sample_raw[sensor_idx, taxel_idx, 1]
