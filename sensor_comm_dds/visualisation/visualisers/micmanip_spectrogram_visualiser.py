...
class MicManipSpectrogramVisualiser(Visualiser):
    def __init__(self, parent=None):
        super().__init__(topic_data_type=Sequence, description="Visualise data from a MicManip sensor in a spectrogram.")
        ...
        self.samples_per_block = 50  # Block is the unit that is published by the senso reader. The value here must agree with the reader script.
        ...
        self.frame_data_publisher = DataPublisher(topic_name="MicManipFrame", topic_data_type=Sequence)
        self.spectrogram_data_publisher = DataPublisher(topic_name="MicManipSpectrogram", topic_data_type=Sequence)
    ...
    def run(self):
        ...
        for sample_block in self.reader.take_iter(timeout=duration(seconds=10)):
            ...
            block_ctr += 1
            if block_ctr == 200:
                logger.info(f'MicManipSpectrogramVisualiser readout frequency: {200 * self.samples_per_block/(time.time() - t_start)}')
                block_ctr = 0
                t_start = time.time()
