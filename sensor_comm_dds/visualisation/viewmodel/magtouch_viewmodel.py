    ...
    def update_view(self, data):
        for row in range(self.view.grid_size[0]):
            for column in range(self.view.grid_size[1]):
                self.view.circle_radii[row][column] = np.sqrt(max(abs(data[row][column][2]) - self.min_norm, 0)/self.max_norm) * self.view.radius_max + self.view.radius_min
